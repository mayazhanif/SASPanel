...
def ajax_domains_by_user():
    """Return domains for a specific user, scoped to the logged-in admin.
    Query param: user_id (int)
    Returns: JSON list of {id, domain} objects.
    """
    ...
    # Scope: domain must belong to user who belongs to this admin
    cursor.execute(
        'SELECT Domain_Id, Domain_Name FROM `domains` '
        'WHERE Is_Deleted=0 AND Domain_Suspended=0 '
        'AND User_id=%s '
