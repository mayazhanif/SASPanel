...
def admin_profile():
    ...
    if check_admin_Login():
        ...
        cursor.execute('SELECT Reg_Date FROM administrator WHERE 1=1')
        ...
        if request.method == 'POST' and 'Name' in request.form:
            ...
            cursor.execute('UPDATE `administrator` SET `Admin_Name` = %s WHERE Admin_id = %s;', (Name))
            mysqlconnection.commit()
            if cursor.rowcount > 0:
                session['Name'] = Name
            ...
            if pass1 == pass2:
                ...
                cursor.execute('UPDATE `administrator` SET `Admin_Password` = %s WHERE Admin_id = %s;', (securePassword))
                mysqlconnection.commit()
                if cursor.rowcount > 0:
...
def admin_updateUser():
    ...
    if check_admin_Login():
        if request.method == 'GET' and request.args.get('userID'):
            ...
            cursor = mysqlconnection.cursor()
            cursor.execute("UPDATE `users` SET `Package_Id` = %s, `User_email` = %s, `User_Password` = %s, `User_Name` = %s WHERE `users`.`User_id` = %s AND Admin_id = %s", (packageID, Email, securePassword, Name, userID))
            mysqlconnection.commit()
            if cursor.rowcount>0:
                flash('User Updated.')
                return safe_referrer_redirect()
...
def admin_deletePackage():
    ...
    if check_admin_Login():
        if request.method == 'GET' and request.args.get('packageID'):
            ...
            cursor.execute("UPDATE `packages` SET `Is_Active` = '0' WHERE `packages`.`Package_Id` =%s AND `Admin_id`=%s",(packageID))
            mysqlconnection.commit()
            if cursor.rowcount>0:
...
def admin_viewDatabases():
    ...
    if check_admin_Login():
        ...
        cursor.execute(
            ...
            'WHERE msqldatabases.Is_Active = 1')
        results = cursor.fetchall()
...
def admin_viewAccounts():
    ...
    if check_admin_Login():
        ...
        cursor.execute(
            ...
            'WHERE ftp_accounts.Is_Active=1;')
        results = cursor.fetchall()
        # Also fetch users list for the 'Add FTP Account' form dropdown
        cursor.execute(
            'SELECT User_id, User_Name, User_email FROM `users` WHERE Is_Deleted=0;')
        users = cursor.fetchall()
        msg = ''
...
def admin_updateAccountPass():
    ...
    if check_admin_Login():
        ...
        if request.method == 'GET' and request.args.get('AccID'):
            ...
            cursor.execute('SELECT FTP_Username FROM `ftp_accounts` INNER JOIN users ON ftp_accounts.User_id = users.User_id WHERE Is_Active=1 AND `ftp_accounts`.`Account_Id`=%s', (AccID))
            row = cursor.fetchone()
            if row is None:
    ...
    if check_admin_Login():
        if request.method == 'GET' and request.args.get('AccID'):
            ...
            cursor.execute(
                ...
                'INNER JOIN users ON ftp_accounts.User_id = users.User_id '
                'WHERE ftp_accounts.Is_Active=1 AND ftp_accounts.Account_Id=%s',
                (AccID)
            )
            row = cursor.fetchone()
...
def admin_addEmail():
    ...
    if check_admin_Login():
        ...
        cursor.execute(
            ...
            'WHERE Is_Deleted=0 ORDER BY User_Name;')
        users = cursor.fetchall()
...
def admin_viewEmail():
    ...
    if check_admin_Login():
        ...
        cursor.execute(
            ...
            'WHERE mail_accounts.Is_Active = 1')
        results = cursor.fetchall()
    ...
    if check_admin_Login():
        ...
        if request.method == 'POST' and 'domainID' in request.form and 'suffix' in request.form :
            ...
            cursor.execute(
                ...
                'WHERE domains.Is_Deleted=0 AND Domain_Id=%s',
                (domainID)
            )
            row = cursor.fetchone()
            if row is None:
...
def admin_error_logs_ajax():
    ...
    if check_admin_Login():
        ...
        cursor.execute(
            ...
            'WHERE domains.Is_Deleted=0 AND domains.Domain_Id=%s',
            (domainID, adminID)
        )
...
def admin_cron_jobs():
    ...
    if check_admin_Login():
        ...
        if request.method == 'POST' and 'userID' in request.form and 'CronTime' in request.form and 'Command' in request.form and 'logFile' in request.form:
            ...
            cursor.execute('SELECT User_id FROM users WHERE User_id=%s AND Is_Deleted=0', (userID))
