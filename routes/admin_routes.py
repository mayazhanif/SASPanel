...
def admin_viewdata():
    if check_admin_Login():
        ...
        cursor.execute('SELECT * FROM ftp_accounts where Is_Active=1')
        ftps = cursor.fetchall()
        countFTP = str(cursor.rowcount)
        cursor.execute('SELECT * FROM mail_accounts where Is_Active=1')
        mails = cursor.fetchall()
        countMails = str(cursor.rowcount)
        cursor.execute('SELECT * FROM msqldatabases where Is_Active=1')
        cursor.fetchall()
        ...
        return render_template('adminFiles/task/viewUser.html', results=users, profileData=profileData)
    else:
        return redirect(url_for('routes.login'))
